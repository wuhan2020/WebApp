import { Logistics, LogisticsDirection, ServiceArea } from '@wuhan2020/rest-api';
...
const initServiceArea: ServiceArea = {
    city: '',
    direction: 'in' as LogisticsDirection,
    personal: false
};
...
@observer
export default class LogisticsEdit extends HTMLElement implements WebCell<LogisticsEditProps> {
    @attribute
    @observable
    accessor dataId = '';
    ...
    async mountedCallback() {
        ...
        const { name, url, serviceArea, remark, contacts } = await logistics.getOne(this.dataId);
    ...
    changeServiceArea(index: number, event: Event) {
        ...
        if (name === 'personal') this.state.serviceArea[index].personal = JSON.parse(value);
    ...
    deleteServiceArea(index: number) {
        ...
        this.state.serviceArea = [...serviceArea.slice(0, index), ...serviceArea.slice(index + 1)];
    }
            ...
            <SessionBox>
                ...
                <form onChange={this.changeText} onSubmit={this.handleSubmit}>
                    ...
                    <FormGroup>
                        ...
                        {serviceArea.map(({ city, direction, personal }, index) => (
                            <InputGroup
                                className="my-1"
                                onChange={(event: Event) => this.changeServiceArea(index, event)}
                            >
                                <FormControl
                                    name="city"
                                    value={city}
                                    placeholder="请输入寄送城市"
                                />
                                <FormControl as="select" name="direction" value={direction}>
                                ...
                                <FormControl as="select" name="personal">
                                    <option selected>是否接受个人捐赠</option>
                                    <option value="true" selected={personal}>
                                        是
                                    </option>
                                    <option value="false" selected={!personal}>
                                        否
                                    </option>
                                </FormControl>

                                <Button variant="primary" onClick={this.addServiceArea}>
                                    +
                                </Button>
                                <Button
                                    variant="danger"
                                    disabled={!serviceArea[1]}
                                    onClick={() => this.deleteServiceArea(index)}
                                >
                                    -
                                </Button>
                            </InputGroup>
                    ...
                    <FormGroup className="mt-3 d-flex flex-column">
                        <Button type="submit" variant="primary" disabled={logistics.uploading > 0}>
                            提交
                        </Button>
                        <Button
                            type="reset"
                            variant="danger"
                            onClick={() => (location.hash = RouteRoot.Logistics)}
                        >
                            取消
                        </Button>
                    </FormGroup>
